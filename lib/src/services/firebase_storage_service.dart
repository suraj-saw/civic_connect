import 'dart:io';
import 'package:firebase_storage/firebase_storage.dart';
import 'package:path/path.dart' as path;

class FirebaseStorageService {
  static final FirebaseStorage _storage = FirebaseStorage.instance;

  /// Upload issue media files
  /// Returns a map with imageUrl, audioUrl, and videoUrl
  static Future<Map<String, String?>> uploadIssueMedia({
    required String issueId,
    File? image,
    File? audio,
    File? video,
  }) async {
    String? imageUrl;
    String? audioUrl;
    String? videoUrl;

    try {
      // Upload image
      if (image != null) {
        imageUrl = await _uploadFile(
          file: image,
          storagePath: 'issues/$issueId/images/${_generateFileName(image)}',
        );
      }

      // Upload audio
      if (audio != null) {
        audioUrl = await _uploadFile(
          file: audio,
          storagePath: 'issues/$issueId/audio/${_generateFileName(audio)}',
        );
      }

      // Upload video
      if (video != null) {
        videoUrl = await _uploadFile(
          file: video,
          storagePath: 'issues/$issueId/videos/${_generateFileName(video)}',
        );
      }

      return {
        'imageUrl': imageUrl,
        'audioUrl': audioUrl,
        'videoUrl': videoUrl,
      };
    } catch (e) {
      throw Exception('Failed to upload media: $e');
    }
  }

  /// Upload resolution image
  static Future<String> uploadResolutionImage({
    required String issueId,
    required File image,
  }) async {
    try {
      return await _uploadFile(
        file: image,
        storagePath: 'resolutions/$issueId/${_generateFileName(image)}',
      );
    } catch (e) {
      throw Exception('Failed to upload resolution image: $e');
    }
  }

  /// Core upload function
  static Future<String> _uploadFile({
    required File file,
    required String storagePath,
  }) async {
    try {
      final Reference storageRef = _storage.ref().child(storagePath);

      // Set metadata
      final SettableMetadata metadata = SettableMetadata(
        contentType: _getContentType(file),
      );

      // Upload file
      final UploadTask uploadTask = storageRef.putFile(file, metadata);

      // Monitor upload progress (optional)
      uploadTask.snapshotEvents.listen((TaskSnapshot snapshot) {
        final progress = (snapshot.bytesTransferred / snapshot.totalBytes) * 100;
        print('Upload progress: ${progress.toStringAsFixed(2)}%');
      });

      // Wait for upload to complete
      final TaskSnapshot snapshot = await uploadTask;

      // Get download URL
      final String downloadUrl = await snapshot.ref.getDownloadURL();

      print('✅ File uploaded successfully: $downloadUrl');
      return downloadUrl;
    } catch (e) {
      print('❌ Upload error: $e');
      rethrow;
    }
  }

  /// Delete issue media (optional - for cleanup)
  static Future<void> deleteIssueMedia(String issueId) async {
    try {
      final Reference issueRef = _storage.ref().child('issues/$issueId');
      final ListResult result = await issueRef.listAll();

      // Delete all files in subdirectories
      for (Reference ref in result.items) {
        await ref.delete();
      }

      // Delete subdirectories
      for (Reference prefix in result.prefixes) {
        await _deleteFolder(prefix);
      }

      print('✅ Deleted all media for issue: $issueId');
    } catch (e) {
      print('❌ Error deleting media: $e');
    }
  }

  /// Delete resolution media (optional)
  static Future<void> deleteResolutionMedia(String issueId) async {
    try {
      final Reference resolutionRef = _storage.ref().child('resolutions/$issueId');
      final ListResult result = await resolutionRef.listAll();

      for (Reference ref in result.items) {
        await ref.delete();
      }

      print('✅ Deleted resolution media for issue: $issueId');
    } catch (e) {
      print('❌ Error deleting resolution: $e');
    }
  }

  /// Helper to delete entire folder
  static Future<void> _deleteFolder(Reference folderRef) async {
    final ListResult result = await folderRef.listAll();

    for (Reference ref in result.items) {
      await ref.delete();
    }

    for (Reference prefix in result.prefixes) {
      await _deleteFolder(prefix);
    }
  }

  /// Generate unique file name with timestamp
  static String _generateFileName(File file) {
    final timestamp = DateTime.now().millisecondsSinceEpoch;
    final extension = path.extension(file.path);
    return '$timestamp$extension';
  }

  /// Get content type based on file extension
  static String _getContentType(File file) {
    final extension = path.extension(file.path).toLowerCase();

    switch (extension) {
      case '.jpg':
      case '.jpeg':
        return 'image/jpeg';
      case '.png':
        return 'image/png';
      case '.gif':
        return 'image/gif';
      case '.webp':
        return 'image/webp';
      case '.mp4':
        return 'video/mp4';
      case '.mov':
        return 'video/quicktime';
      case '.avi':
        return 'video/x-msvideo';
      case '.m4a':
        return 'audio/mp4';
      case '.mp3':
        return 'audio/mpeg';
      case '.wav':
        return 'audio/wav';
      default:
        return 'application/octet-stream';
    }
  }
}