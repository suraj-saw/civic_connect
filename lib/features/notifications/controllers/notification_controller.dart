...
class NotificationController extends GetxController {
  ...
  void _onIssuesSnapshot(QuerySnapshot snapshot) {
    ...
    // Push status updates to CivicBot (#11 - Notification Integration)
    _notifyChatbot(extracted);
  ...
  /// Push new unread notifications to CivicBot as in-chat messages.
  void _notifyChatbot(List<NotificationItem> items) {
    if (!Get.isRegistered<ChatbotController>()) return;
    final chatCtrl = Get.find<ChatbotController>();
    for (final item in items) {
      if (item.isRead) continue;
      final id = _entryId(item.issueId, item.timestamp);
      if (_notifiedChatIds.contains(id)) continue;
      _notifiedChatIds.add(id);
      chatCtrl.addNotificationMessage(
        item.categoryId.isNotEmpty ? item.categoryId : item.issueId,
        item.status,
      );
    }
  }
