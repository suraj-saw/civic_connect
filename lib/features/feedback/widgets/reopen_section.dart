...
class ReopenSection extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Obx(() => ctrl.reopenSucceeded.value ? _successBanner(context) : _form(context));
  }
    ...
    return Container(
      ...
      child: const Row(children: [
        ...
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text('Issue Reopened', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.orange, fontSize: 14)),
          SizedBox(height: 2),
          Text('Your proof has been submitted. The department must resolve this again.',
        ...
        children: [
          Container(
            ...
            child: const Row(children: [
              ...
              SizedBox(width: 8),
              Expanded(child: Text('Issue Not Resolved — Submit Proof',
          ...
          Padding(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('At least one photo and a description are required.',
                    style: GoogleFonts.inter(fontSize: 12, color: Colors.grey.shade700)),
                const SizedBox(height: 16),
                _sectionLabel('Photos', Icons.camera_alt_outlined, required: true),
                const SizedBox(height: 8),
                _photoSection(),
                const SizedBox(height: 16),
                _sectionLabel('Video', Icons.videocam_outlined, required: false),
                const SizedBox(height: 8),
                _videoSection(),
                const SizedBox(height: 16),
                _sectionLabel('Voice Note', Icons.mic_outlined, required: false),
                const SizedBox(height: 8),
                _audioSection(),
                const SizedBox(height: 16),
                _sectionLabel('Description', Icons.edit_outlined, required: true),
                const SizedBox(height: 8),
                TextField(
                ...
                Obx(() => SizedBox(
                  ...
                  child: ElevatedButton.icon(
                    onPressed: ctrl.isReopening.value ? null : ctrl.submitReopen,
                    icon: ctrl.isReopening.value
                        ? const SizedBox(width: 16, height: 16, child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white))
                        : const Icon(Icons.recycling_rounded, size: 18),
                    label: Text(ctrl.isReopening.value ? 'Submitting...' : 'Reopen Issue'),
  ...
  Widget _sectionLabel(String label, IconData icon, {required bool required}) {
    return Row(children: [
      ...
      if (required) ...[
        ...
        Text('  (optional)', style: TextStyle(fontSize: 11, color: Colors.grey.shade500)),
    ]);
  }
  ...
  Widget _photoSection() {
    return Obx(() => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        GridView.builder(
          ...
          itemBuilder: (_, i) => Stack(fit: StackFit.expand, children: [
            ClipRRect(borderRadius: BorderRadius.circular(8),
                child: Image.file(File(ctrl.reopenImages[i].path), fit: BoxFit.cover)),
      ...
      OutlinedButton.icon(
        ...
        label: Text(ctrl.reopenImages.isEmpty ? 'Take Photo' : 'Add Another Photo'),
      ...
      if (ctrl.reopenImages.isEmpty)
        Padding(padding: const EdgeInsets.only(top: 5),
            child: Text('At least 1 photo required', style: TextStyle(fontSize: 11, color: Colors.red.shade400))),
    ]));
  }
      ...
      if (video != null) {
        return Container(
          ...
          child: Row(children: [
            ...
            GestureDetector(onTap: () => ctrl.reopenVideo.value = null, child: const Icon(Icons.close, size: 18, color: Colors.grey)),
          ]),
        );
      }
  ...
  Widget _audioSection() {
    return Obx(() {
      ...
      if (audio != null) {
        return Container(
          ...
          child: Row(children: [
            ...
            const SizedBox(width: 8),
            const Expanded(child: Text('Voice note recorded', style: TextStyle(fontSize: 13))),
            GestureDetector(onTap: () => ctrl.reopenAudio.value = null, child: const Icon(Icons.close, size: 18, color: Colors.grey)),
          ]),
        );
      }
      ...
      return OutlinedButton.icon(
        ...
        icon: Icon(isRecording ? Icons.stop_circle_outlined : Icons.mic_outlined, size: 18),
        label: Text(isRecording ? 'Stop Recording' : 'Record Voice Note'),
        style: OutlinedButton.styleFrom(
          foregroundColor: isRecording ? Colors.red.shade900 : Colors.red.shade700,
