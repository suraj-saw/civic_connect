...
class IssueDetailAdminPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final categoryCtrl =
        Get.isRegistered<IssueCategoryController>()
          ...
          return SingleChildScrollView(
            ...
            child: Column(
              children: [
                ...
                AdminIssueMediaSection(
                  data: data,
                ).animate().fadeIn(delay: 120.ms),
                if (status == 'resolved')
                  _ResolutionProofSection(
                    data: data,
  ...
  void _showReassignDialog(
    ...
    String fromDept,
    IssueCategoryController categoryCtrl,
  ) {
  ...
  Future<void> _submitReassign(
    ...
    try {
          ...
          'timeline': FieldValue.arrayUnion([
            {
              'status': 'reassigned',
              'message':
                  'Reassigned from ${fromDept.toUpperCase()} to ${toDept.toUpperCase()}. Reason: $reason',
              'updatedBy': uid,
              'updatedByEmail': adminEmail,
              'timestamp': Timestamp.now(),
            },
      ...
      AppSnackbar.show(
        'Success',
        'Issue reassigned to ${toDept.toUpperCase()}.',
        snackPosition: SnackPosition.BOTTOM,
      );
    } catch (e) {
      AppSnackbar.show(
        'Error',
        'Failed to reassign: $e',
        snackPosition: SnackPosition.BOTTOM,
      );
    }
  }
...
class _ResolutionProofSection extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final imageUrls =
        (resolution['proofImageUrls'] as List<dynamic>? ?? [])
            .whereType<String>()
            .toList();
          ...
          children: [
            ...
            if (_video != null)
              Container(
                ...
                child: Row(
                  children: [
                    Icon(Icons.videocam, color: cs.secondary, size: 20),
                    const SizedBox(width: 8),
                    Expanded(
                      child: Text(
                        'Video recorded',
            ...
            SizedBox(
              ...
              child: ElevatedButton.icon(
                ...
                label: Text(
                  _isSubmitting ? 'Uploading...' : 'Submit Resolution',
                ),
              ),
            ),
          ],
